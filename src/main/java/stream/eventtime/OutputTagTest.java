...
public class OutputTagTest {
    ...
    public static void main(String[] args) throws Exception {
        ...
        SingleOutputStreamOperator<String> window = waterMarkDataStream.keyBy("topic").timeWindow(Time.seconds(3)).sideOutputLateData(outputTag).apply(new WindowFunction<Message, String, Tuple, TimeWindow>() {
            @Override
            public void apply(Tuple tuple, TimeWindow window, Iterable<Message> input, Collector<String> out) throws Exception {
        ...
        outputTagDataStream.process(new ProcessFunction<Message, Object>() {
            @Override
            public void processElement(Message value, Context ctx, Collector<Object> out) throws Exception {
                out.collect(value);
                //ctx.output(outputTag, "sideout-" + value.toString());
